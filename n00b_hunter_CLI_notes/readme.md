...
Browse and search **2,693 commands** across **18 attack modules**, each with MITRE ATT&CK IDs:

| Module | Sections | Commands | Coverage |
|--------|----------|----------|----------|
| `Active Directory` | 40 | 287 | Kerberoasting, AS-REP roasting, DCSync, ACL abuse, Golden/Silver Ticket, AD CS (ESC1–11), RBCD, ADIDNS takeover, BloodHound, LAPS, Trust attacks, LLMNR/NBT-NS poisoning, PrintNightmare, PetitPotam, NoPac |
| `Web Application` | 38 | 268 | SQLi, XSS, SSRF, XXE, SSTI, LFI/RFI, command injection, file upload bypass, deserialization, HTTP smuggling, WAF bypass, CMS exploitation, Tomcat/Jenkins/Splunk RCE |
...
| `Linux Systems` | 27 | 212 | SUID/SGID, kernel exploits, polkit/D-Bus abuse, cloud IMDS exploitation, credential hunting, container breakout, LXD escape, NFS privesc, Metasploit Framework, shell stabilization |
...
| `Windows Systems` | 20 | 180 | Privesc, LSASS evasion, credential hunting, browser credential extraction, service misconfiguration, WMI persistence, COM hijacking, AlwaysInstallElevated, UAC bypass |
| `Network Infrastructure` | 20 | 172 | NTLM relay, LLMNR/NBT-NS poisoning, SMB/RDP/FTP/SMTP attacks, nmap reference, ARP MITM, VLAN hopping, DNS attacks, wireless, IPv6 |
| `Amazon Web Services` | 23 | 112 | IAM privesc paths, GuardDuty evasion, Lambda Function URL abuse, S3, EC2, CloudTrail evasion |
| `Containers & Kubernetes` | 18 | 98 | Docker escape, K8s RBAC escalation, image supply chain attacks, etcd extraction, Helm secrets |
| `Database Attacks` | 19 | 121 | MSSQL, MySQL, PostgreSQL, cloud managed DB attacks, credential exfiltration, Redis, MongoDB, xp_cmdshell, linked servers |
| `Google Cloud` | 20 | 100 | IAM, service account impersonation, compute metadata exploitation, GKE, Cloud Run, BigQuery |
| `Artificial Intelligence` | 22 | 95 | Prompt injection, RAG poisoning, function calling abuse, LLM API key exposure, agent hijacking, supply chain ML |
| `Microsoft Azure` | 18 | 90 | AAD, conditional access bypass, managed identity abuse, Azure OpenAI abuse, DevOps pipelines, Key Vault |
...
**2,224 detection queries** across two libraries — SOC SIEM and Threat Hunting — each covering five platforms. All detection categories are mapped to MITRE ATT&CK technique IDs.
...
| **Elastic (KQL)** | 42 | 395 | RBCD/ADIDNS, PwnKit/polkit, Discord/Telegram C2, ECS-mapped Sysmon fields |
...
Multi-level search with AND logic and keyword highlighting at every menu depth:
...
| **Offensive (cross-module)** | `[s]` in Offensive module list | All 18 offensive modules |
...
n00b_hunter/
│
├── n00b_hunter.py                      ← Main script (v3.6)
├── ReamMe.md
│
...
[s] is available at every menu level:
  Offensive module list        → search across ALL 18 offensive modules
...
|----------|----------|----------|
| Active Directory | 40 | 287 |
| Web Application | 38 | 268 |
| Linux Systems | 27 | 212 |
| Sliver C2 | 14 | 167 |
| Android | 29 | 160 |
| Cobalt Strike C2 | 14 | 152 |
| Havoc C2 | 14 | 144 |
| Social Media Offensive | 19 | 140 |
| Network Infrastructure | 20 | 172 |
| Windows Systems | 20 | 180 |
| Amazon Web Services | 23 | 112 |
| Database Attacks | 19 | 121 |
| Evasion | 22 | 121 |
| Containers & Kubernetes | 18 | 98 |
| Google Cloud | 20 | 100 |
| Artificial Intelligence | 22 | 95 |
| Microsoft Azure | 18 | 90 |
| Social Engineering | 13 | 74 |
| **Offensive Total** | **390** | **2,693** |
| | | |
...
### v3.6 — Current
- **HackTheBox Academy cheatsheet integration** — 24 HTB Academy cheatsheets parsed and merged into the offensive TTP database, adding 436 net-new commands across 6 modules:
  - **Active Directory** (+94 commands, +8 sections): LLMNR/NBT-NS poisoning (Responder/Inveigh), Living-off-the-Land AD enumeration (Get-ADDomain, PowerView, windapsearch, Snaffler), GPP password recovery, PrintNightmare (CVE-2021-34527), PetitPotam, NoPac, privileged SQL access via AD; added commands to password_policy, password_spraying, kerberoasting, asreproasting, dcsync, forest_trust_attacks, gpo_abuse
  - **Linux Systems** (+79 commands, +7 sections): LXD container escape, NFS no_root_squash privesc, shell stabilization & TTY upgrade, full Metasploit Framework + Meterpreter command reference; added commands to initial_enum, privilege_escalation
  - **Web Application** (+79 commands, +2 sections): Attacking Java applications (Tomcat/Jenkins/Splunk RCE), Groovy script console payloads, msfvenom WAR files; added commands to recon_enum, web_fuzzing, sql_injection, xss, lfi_rfi, cms_exploitation
  - **Network Infrastructure** (+90 commands, +6 sections): Comprehensive nmap flag reference, attacking FTP/SMB/RDP/email services, footprinting services (DNS, SNMP, NFS, MSSQL, Oracle, SSH, WMI)
  - **Windows Systems** (+62 commands, +1 section): Credential hunting (PSReadLine, Chrome, LaZagne, SessionGopher, wireless), added commands to initial_enum, privilege_escalation, credential_harvesting
  - **Database Attacks** (+32 commands, +1 section): MySQL attacks (LOAD_FILE, INTO OUTFILE webshell), added commands to mssql_enum
- **Deduplication pass** — 14 duplicate commands removed across 5 files after integration
- **Stats corrected** — grand total updated to 769 sections, 4,917 commands
